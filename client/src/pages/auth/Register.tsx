...
function getSchema(t: (key: string) => string) {
  return z.object({
    name: z.string().min(2, t('auth.register.name')),
    email: z.string().email(t('auth.register.email')),
    password: z.string().min(6, t('auth.register.password')),
...
export default function Register() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    try {
      setError('');
      setLoading(true);
      await registerUser({ ...data, language: i18n.language });
      navigate('/dashboard');
    } catch (err: any) {
